 ...
 * 无自设分辨率上限 —— 只受 CCMRAM 实际空间约束，整片装不下由链接期报错）。
 *
 ...
 *
 *  守卫性质分三类，**没有一类是「自设分辨率 / CCM 预算上限」**（该自设 16KB 预算
 *  已于第三十二轮删除，改为下方「必须装进 CCMRAM 区域 64KB」这一内存物理守卫）：
 *    ① 类型宽度：模块数装 uint8_t、状态数装 uint8_t、像素数 / 帧位数装 uint16_t；
 *    ② 派生一致性：单模块恰 8 块 = 128 位、链段数 = 4×M、状态数组 = 组数×每帧时钟；
 *    ③ 物理事实：HUB75 通道数（2×COLS ≤ 10）、数据线固定 4 条、CCMRAM 区域 64KB。
 * ================================================================ */
...
/** 本模组 CCMRAM 占用（CCMRAM 共 64KB，与 CQ / 贵州治超 / 云南治超队列、静态任务栈共用）：
 *  chain_dst 2×4M×128 + pixel_map 256M + 帧状态数组 128M + 合并写表 8×2×16×组数
 *  ⇒ 通式 1408·M + 256·MODULE_COLS（1×1 = 1664B、1×2 = 3328B、8×2 = 23040B）。 */
