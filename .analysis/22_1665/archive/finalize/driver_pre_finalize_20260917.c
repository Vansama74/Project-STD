 ...
 *
 * 料号 2200001665：列驱动 MBI5034B（16 通道恒流下沉 + 16 位移位寄存器，SDO→SDI 级联），
 ...
 *
 * 接线模型（`_22_1665_HUB_WIRING`，默认 2；2026-09-17 现场判定 + 确认显示正常）：
 *   2 = 模型 B（每列一口独立线）：每个模块列（= 一个 HUB 口）各占一组 4 根数据脚，
 *       组 g 取兄弟驱动（1_263 / 22_1703）的通道对 2g / 2g+1 —— 组 0 = R1/G1/R2/G2
 *       （PG9/PG10/PG15/PB6，现场逐脚定标）、组 1 = R3/G3/R4/G4（PB8/PB9/PE1/PE2）；
 *       各组并行（共享 CLK/LAT/OE），组内横向模块同线级联，每帧 128 × MODULE_ROWS 个时钟；
 *       MODULE_ROWS = 1 时每组一块 ⇒ HUB1 显逻辑上半、HUB2 显逻辑下半（合起来才是整屏）。
 *   1 = 模型 A（同线级联）：一组 4 根脚把各模块同名链段串成一条链，模块 m 占帧内时钟
 *       [(M-1-m)·128, (M-m)·128)、每帧 128 × M；贴数据脚那块（链首）收末 128 位 = 模块 M-1。
 *       第二十四/二十五轮现场取证形态，代码路径逐字节保留，供现场 A/B 与历史溯源。
 *   两模型在 M = 1 时等价（1×1 输出逐字节一致，宿主差分证明）；详规见 doc/01 §0.3/§0.10。
 *
 * 数据流：prepare 逐时钟位 p 合成状态字节 `state[组][p]`（**bit j = 组内第 j 条数据线的电平**；
 * 红线段取像素 R 位、绿线段取 G 位，故黄 = 两条都亮）→ scan 逐位按组查合并写表写各端口 BSRR
 * （1×2 时每时钟 4 次：G/B + B/E）→ CLK 脉冲；帧末由基类在 OE 消隐窗口内打 LAT。
 *
 ...
 *
 * CCMRAM（模型 B 通式 **1408·M + 256·MODULE_COLS**；1×1 = 1664B、1×2 = 3328B；
 * 模型 A 通式 1408·M + 384；上限 16KB 由 `_Static_assert` 守卫）：
 *   chain_dst 2×4M×128 + pixel_map 256M + 帧状态数组 128M + 合并写表 8×2×16×组数。
 *
 * 开关宏（全部 `#ifndef` 守卫，可命令行 `-D` 覆盖）：`_22_1665_HUB_WIRING`（1 = 模型 A /
 * 2 = 模型 B）、`_22_1665_BLUE_AS_LIT`（双色屏无蓝 die，蓝分量并入红绿）、
 * `_22_1665_DATA_ACTIVE_HIGH`（数据极性：出现补色图案时改 0）、`_22_1665_CHAIN_HEAD_IS_MODULE0`
 * （链首口径：模型 A = 整屏链首 / 模型 B = 组内横向链首；默认 0 ⇒ 链首装组内最后一块，
 * MODULE_ROWS = 1 时无影响）。
...
/* ---- 屏体尺寸（**唯一几何入口**；可 `-D` 覆盖，改动后由「编译期防御」把关）---- */
...
#define _22_1665_PORT_MAX       (3U)  /* 模型 A：4 线分布的端口槽数上限（本板 PG+PB 两个，取 3 留余量） */
#define _22_1665_GROUP_PORT_MAX (2U)  /* 模型 B：单组 4 线分布的端口槽数上限（本板每组 2：G+B / B+E） */
...
/* 帧状态数组长度 = 链位总数 = 128 × M（模型 B 下 = 组数 × 每帧时钟数，见断言） */
#define _22_1665_FRAME_BITS (_22_1665_SEGMENT_BITS * _22_1665_MODULE_TOTAL)
/* 链段数 = 4 × M（模型 A：4 条线串成 4M 段；模型 B：每列 4M 段分摊到各组） */
#define _22_1665_CHAIN_COUNT (_22_1665_LINE_COUNT * _22_1665_MODULE_TOTAL)

/* ---- 行为开关（可命令行 `-D` 覆盖）---- */

/** 接线模型：1 = 模型 A（同线级联，单组线串各模块）；2（默认）= 模型 B（每列一口独立线）。
 *  现场 2026-09-17 判定 = 模型 B（两块模组各插一口：HUB1 只出「模块 1」内容、HUB2 全黑）——
 *  默认按模型 B；模型 A 保留为可编译 A/B 路径（M = 1 时两者等价）。 */
#ifndef _22_1665_HUB_WIRING
#define _22_1665_HUB_WIRING (2U)
#endif
 ...
 *  （模型 A：模块 M-1；模型 B：组内行尾模块 mx = MODULE_ROWS-1，与模型 A 的「链首 = M-1」
 *   口径同构）；1 = 链首装组内**第一个**模块（mx = 0 / 模型 A 的模块 0）。
 *  只影响「链上多于一块模组」的口径（模型 B 下 MODULE_ROWS = 1 时每组仅一块，无影响）；
 *  现场首接多模块时用它做 A/B（见 doc/01 §0.10）。 */
 ...
 * ================================================================ */
/* 接线模型只允许 1 / 2（非法值直接报错，不静默取默认） */
_Static_assert(_22_1665_HUB_WIRING == 1U || _22_1665_HUB_WIRING == 2U,
               "22_1665: HUB_WIRING must be 1 (chained) or 2 (per-column lines)");
...
/* 每组数据线为 4 条（R/G × 上/下半屏）；链段数 = 4 × 模块总数（模型 A 串成 4M 段） */
...
/* 模型 B 需要 2 × MODULE_COLS 个通道；超出通道表即编译报错（不允许静默少驱动一列） */
_Static_assert(_22_1665_HUB_WIRING == 1U || 2U * _22_1665_MODULE_COLS <= HUB75_CHANNEL_MAX,
...
/* ---- 接线模型派生：组数 / 每帧时钟 / 组内模块数 / 模块归属 / 链首块号 ---- */
#if _22_1665_HUB_WIRING == 2
/* 模型 B：每列一口——组数 = 模块列数；每帧时钟 = 128 × 每列模块数（各组并行、共享时钟） */
...
#else
/* 模型 A：同线级联——单组；每帧时钟 = 128 × 模块总数；组内模块 = 全部模块（行主序） */
#define _22_1665_GROUP_COUNT          (1U)
#define _22_1665_CLOCKS               (_22_1665_SEGMENT_BITS * _22_1665_MODULE_TOTAL)
#define _22_1665_GROUP_MODULE_COUNT   (_22_1665_MODULE_TOTAL)
#define _22_1665_MODULE_OF_GROUP(g, local) ((void)(g), (uint16_t)(local))
#endif
/* 状态数组长度必须等于「组数 × 每帧时钟数」（模型 B 按组分段、模型 A 退化单段） */
...
/* 链首块号：链上最后移入的 128 位停在链首（模型 A 的整条链 / 模型 B 的组内链） */
 ...
 *  chain_dst 2×4M×128 + pixel_map 256M + 帧状态数组 128M + 合并写表（模型 B：8×2×16×组数；
 *  模型 A：8×3×16）。模型 B 通式 = 1408·M + 256·MODULE_COLS；模型 A 通式 = 1408·M + 384。 */
#if _22_1665_HUB_WIRING == 2
#define _22_1665_BSRR_SLOTS (_22_1665_GROUP_PORT_MAX * _22_1665_GROUP_COUNT)
#else
#define _22_1665_BSRR_SLOTS (_22_1665_PORT_MAX)
#endif
...
/* 本模组 CCMRAM 预算上限 16KB（模型 B：1×1 为 1664B、1×2 为 3328B） */
...
#if _22_1665_HUB_WIRING == 2
/** 合并写表（模型 B）：[组][端口槽][4 位状态] → 该端口 32 位 BSRR 字（init 期生成，热路径只查表） */
...
#else
/** 合并写表（模型 A）：[端口槽][数据线状态] → 该端口 32 位 BSRR 字（1 组 4 线共 PORT_MAX 槽） */
[[gnu::section(".ccmram")]] static pl_hub75_bsrr_t
    _22_1665_bsrr_tab[_22_1665_PORT_MAX][_22_1665_STATES];
static uint8_t _22_1665_bsrr_port_cnt; /* 实际参与输出的端口槽数（≤ PORT_MAX） */
#endif
 ...
 *
 *  逐组（模型 B：一个模块列 = 一个 HUB 口）填它那 CLOCKS 个时钟位；组内模块的 128 位段落在
 *  `[(链内模块数-1-local)·128, ...)`（先移入者停在链尾，链首收末 128 位——模型 A 时为整条链，
 *  模型 B 时为该组那条链）。状态字节 **bit j = 组内第 j 条数据线**在该时钟位的电平
 *  （恒 4 位 / 16 状态，与模块数无关）；链段 (m, j) 的像素来源 = 落点表 _22_1665_chain_dst。
 ...
 *  （模型 B 每时钟 = 4 次合并 BSRR 写：组 0 的 G/B + 组 1 的 B/E；模型 A = 2 次）、
 *  给建立裕量、再打 CLK 脉冲。先移入的位停在链尾，故按时钟位 p 递减遍历；
 *  帧末由基类在 OE 消隐窗口内打 LAT。
 * ================================================================ */
...
{
    pl_hub75_bsrr_flush(&_22_1665_bsrr_tab[0][st]);
    if (_22_1665_bsrr_port_cnt > 1U)
        pl_hub75_bsrr_flush(&_22_1665_bsrr_tab[1][st]);
    if (_22_1665_bsrr_port_cnt > 2U)
        pl_hub75_bsrr_flush(&_22_1665_bsrr_tab[2][st]);
}
...
#if _22_1665_HUB_WIRING == 2
/** @brief 建合并写表（模型 B）：逐组把 4 根线按 GPIO 端口分组，预计算「状态 → 端口 BSRR 字」 */
static void _22_1665_build_bsrr_table(void)
{
    for (uint8_t g = 0U; g < _22_1665_GROUP_COUNT; g++) {
        ...
        /* ② 逐状态：本组参与线「亮 → 置位、灭 → 复位」（极性由 DATA_ACTIVE_HIGH 决定） */
...
#else
/** @brief 建合并写表（模型 A）：4 条数据线按 GPIO 端口分组，预计算「状态 → 端口 BSRR 字」 */
static void _22_1665_build_bsrr_table(void)
{
    ...
    for (uint8_t j = 0; j < _22_1665_LINE_COUNT; j++) {
        line_slot[j]       = 0xFFU;
        GPIO_TypeDef *port = _22_1665_lines[0][j].line->port;
        uint8_t slot       = 0xFFU;
    ...
    for (uint16_t st = 0U; st < _22_1665_STATES; st++) {
        for (uint8_t s = 0; s < port_cnt; s++) {
            _22_1665_bsrr_tab[s][st].port = ports[s];
            _22_1665_bsrr_tab[s][st].val  = 0U;
        }
        for (uint8_t j = 0; j < _22_1665_LINE_COUNT; j++) {
            if (line_slot[j] == 0xFFU)
                continue;
            const uint32_t mask = (uint32_t)_22_1665_lines[0][j].line->pin;
            const bool on = (((st >> j) & 1U) != 0U) ^ (_22_1665_DATA_ACTIVE_HIGH == 0U);
            _22_1665_bsrr_tab[line_slot[j]][st].val |= on ? mask : (mask << 16U);
        }
    }

    _22_1665_bsrr_port_cnt = port_cnt; /* 有数据线的槽必然非空（每槽至少一条线，状态里会点亮它） */
}
#endif
