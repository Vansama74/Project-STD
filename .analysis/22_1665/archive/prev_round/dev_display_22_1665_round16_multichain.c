 ...
 *    _22_1665_CHAIN_PROBE   链序探针 0..8（默认 0 = 关；开启后 scan 忽略显示内容）
 ...
 *
 *  ★★ 下面 4 行的 `line_mask` 与区域**都是占位值**（按「R/G 成对覆盖上/下半屏」推定），
 *     未经现场标定；标定前屏上内容可能出现在错误的位置/颜色，这是预期行为。★★
 * ================================================================ */
typedef struct {
...
static const _22_1665_chain_t _22_1665_chains[_22_1665_CHAIN_COUNT] = {
#if _22_1665_MULTI_CHAIN
    /* 链 0 ── ★待现场标定（数据脚 / 角色 / 区域均为占位值）★ ── 占位：上半屏 红 */
    ...
    /* 链 2 ── ★待现场标定★ ── 占位：下半屏 红 */
    _22_1665_ROW("C2", _22_1665_LINE_B1, _22_1665_ROLE_RED, 0U, _22_1665_HALF_H,
                 _22_1665_SCREEN_W, _22_1665_HALF_H),
#endif
#if _22_1665_CHAIN_COUNT > 3
    /* 链 3 ── ★待现场标定★ ── 占位：下半屏 绿 */
    _22_1665_ROW("C3", _22_1665_LINE_R2, _22_1665_ROLE_GREEN, 0U, _22_1665_HALF_H,
                 _22_1665_SCREEN_W, _22_1665_HALF_H),
#endif
...
/* 需要「脚名 / 端口字母 / 位号」标签的设施（自检打印或探针 8 打印） */
#if _22_1665_RTT_DIAG && (_22_1665_PIN_SELFTEST_ON || (_22_1665_CHAIN_PROBE == 8))
...
_Static_assert(_22_1665_CHAIN_PROBE <= 8U, "探针模式只能取 0..8");
 ...
 * ================================================================ */
static uint16_t _22_1665_region_offset(uint8_t x0, uint8_t y0, uint8_t blk_cols, uint16_t p)
{
...
static void _22_1665_build_chain_dst(uint8_t i)
{
    ...
    for (uint16_t p = 0; p < _22_1665_FRAME_BITS; p++)
        dst[p] = _22_1665_region_offset(c->x0, c->y0, blk_cols, p);
}
...
static void _22_1665_chain_init(void)
{
    ...
    for (uint8_t i = 0; i < _22_1665_CHAIN_COUNT; i++) {
...
#if _22_1665_RTT_DIAG
        {
            ...
            if (s_chain_ok[i]) {
                SEGGER_RTT_printf(
                    0, "[22_1665] chain%u %-3s lines=0x%03X role=%s region=(%u,%u,%u,%u) dst=%s\n",
                    ...
                    (c->dst != nullptr) ? "explicit" : "default");
            } else {
...
/** 半周期帧数（TIM3 = 500µs/帧 → 500ms = 1000 帧） */
...
/** @brief 探针 8 的「已知链」区域取链 0 的区域（多链 = 已知链半屏；兼容 = 整屏）。
...
static void _22_1665_probe8_draw_digit(uint8_t k)
{
    ...
    const int w  = 5;
    const int h  = 7;
    int x0 = (int)rx + (((int)rw - w) / 2);
    int y0 = (int)ry + (((int)rh - h) / 2);
    if (x0 < (int)rx) x0 = (int)rx;
    if (y0 < (int)ry) y0 = (int)ry;

    for (int r = 0; r < h; r++) {
        const int y = y0 + r;
        if (y >= (int)_22_1665_SCREEN_H || y >= ((int)ry + (int)rh))
            break;
        for (int c = 0; c < w; c++) {
            const int x = x0 + c;
            if (x >= (int)_22_1665_SCREEN_W || x >= ((int)rx + (int)rw))
                break;
