 ...
 *
 * 料号 2200001665；列驱动 MBI5034B（16 通道恒流 + 16 位移位寄存器 + 输出栓锁，SDO→SDI 级联）。
 * **2026-09-17 现场确认：显示正常**（此前 `prepare` 的逐链掩码缺陷已修复，见文末「历史」）。
 *
 * ── 硬件口径（as-built）────────────────────────────────────────────────────
 *  屏面 **16×16 像素** = 256 红 die + 256 绿 die = 512 颗 LED = **4 条链 × 8 片 MBI5034B**
 *  （每条链 128 位）。**静态扫描**（`scan_lines = 1`，不用行址线）；四条链共享 CLK/LAT，
 *  数据脚各自独立（第二十轮现场探针 8 逐脚读数定标）：
 *
 *      链 0 = R1 (PG9)  上半屏 红       链 1 = G1 (PG10) 上半屏 绿
 *      链 2 = R2 (PG15) 下半屏 红       链 3 = G2 (PB6)  下半屏 绿
 *
 *  B1/B2/A/B/C/D 六根脚现场逐脚实测「无链」（闪 + 纹理全程无反应）⇒ 链表一个都不占用。
 *
 * ── 数据流 ────────────────────────────────────────────────────────────────
 *  像素写入 `pixel_map`（16×16 颜色索引）后：
 *   · prepare：逐时钟位 p（0..127）把各链的电平拼成**状态字节** `frame_state[p]`
 *     （bit i = 第 i 条链该位电平）—— 红链取像素 R 位、绿链取 G 位（黄 = 两条都亮；
 *     蓝按 `_22_1665_BLUE_AS_LIT` 并入红绿）；
 *   · scan：逐位取状态 → 查**合并写表**按 GPIO 端口写 BSRR（≤3 次）→ CLK 脉冲；
 *     帧末由基类在 OE 消隐窗口内打 LAT；
 *   · set_row：静态单扫（row 恒 0）；**被选作数据脚的地址脚由 scan 独占，本函数不碰**。
 *
 * ── 唯一标定块：链表（§二 的 `_22_1665_chains[]`）────────────────────────────
 *  每链一行 = 数据脚掩码 / 颜色角色 / 覆盖区域 (x0,y0,w,h) / 落点镜像旗标 / 可选显式落点表。
 *  默认落点规则（第十二轮现场反解、已逐位确认）：链位 p → 片 c = p/16、片内级序 i = p%16；
 *  块 bc = (块列数-1) - (c % 块列数)、br = c / 块列数；块内 dx = 3-(i%4)、dy = ((i/4)+2)%4。
 *  改脚 / 改区域 / 改朝向**只改链表一处**；下半屏旋屏用 `_22_1665_ROW_XF(..., _22_1665_XF_ROT180)`。
 *
 * ── 开关（全部 `#ifndef` 守卫，可命令行 `-D` 覆盖）────────────────────────────
 *   _22_1665_MULTI_CHAIN   1 = 多链独立位流（默认）；0 = 单链兼容（16×8、候选脚同流、
 *                          「非黑即亮」；**回退 / 现场对照用**，CCM 与第十五轮逐字节一致）
 *   _22_1665_CHAIN_COUNT   链数（默认 4；兼容模式强制 1）
 *   _22_1665_DATA_LINES    探针候选脚掩码（默认 0x3FF 十脚全开；兼容模式 = 同流脚）
 *   _22_1665_CHAIN_PROBE   链序探针 0..9（默认 0 = 关；开启后 scan 忽略显示内容，见 §三）
 *   _22_1665_BLUE_AS_LIT / _22_1665_DATA_ACTIVE_HIGH / _22_1665_BLK_COLS（仅兼容模式）
 *   _22_1665_PIN_SELFTEST / _22_1665_RTT_DIAG   上电引脚回读自检 / RTT 输出（默认均 1）
 *
 * ── 探针（标定设施，默认全关；现场调其它问题屏仍可用）──────────────────────────
 *   7 = 逐脚标识（每块点数 = k+1，**对链内位序置换不变**）
 *   8 = 逐脚闪烁 + 纹理（判「真被驱动」还是「悬空 / 弱耦合」）
 *   9 = 逐链 × 逐脚内容扫描（**标定首选**：一次烧录读全「脚 ↔ 区域 / 颜色 / 朝向」）
 *   1 走点 / 2 区间全亮 / 3 前半链 / 4 片锚点 / 5 全链点亮 / 6 等价 5
 *   注：**多链模式下探针 1..6 只驱动链 0**（状态位只到 bit0）；「十脚恒定全 1」仅兼容模式成立。
 *
 * ── 设计约束 ──────────────────────────────────────────────────────────────
 *  · 只改本文件即可（基类 `dev_display.c` / `pl_hub75` / `app_test` / `app_boot` 零改动）；
 *  · 未参与链表的脚一个位都不写（保持静默电平，互不干扰）；同一根脚被两条链声明
 *    = 位流按位「或」叠加（物理上喂不了两份数据）→ init 期 RTT 告警；
 *  · 多链必须**并行喂**（每时钟位把所有链的位一起写下去）；分时喂会让其余链吃进垃圾。
 *
 * ── 历史（追溯用，详见 doc）────────────────────────────────────────────────
 *  第十二 ~ 二十一轮的演进（现场反解落点模型、探针逐脚定位、四链定标、`prepare` 逐链掩码
 *  缺陷修复）见 `doc/01_显示系统/22-1665模组驱动分析与迁移记录.md`（§0 = as-built 摘要，
 *  文末「历史轮次」= 各轮原始记录）；宿主脚本与各口径预测图见 `.analysis/22_1665/`
 *  （索引 = 同目录 `README.md`）。
 */
...
#include "SEGGER_RTT.h"  /* RTT 诊断：链表自检行 / 引脚回读自检 / 探针 8/9 停留提示
                          * （全部输出由 `_22_1665_RTT_DIAG` 单一开关控制；关时零成本） */
 ...
 *
 *  现场标定（两条判据 + 一张表，详见 doc/01 §0.3「标定方法」与 §0.6「RTT 判读表」）：
 ...
 *     `_22_1665_ROW_XF(..., _22_1665_XF_ROT180)`（或对应镜像旗标）。详见 doc/01 §0.8 遗留项 1。★★
 * ================================================================ */
typedef struct {
