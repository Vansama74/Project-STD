 ...
 *     默认 4 行 = R1/G1（上半红/绿）+ R2/G2（下半红/绿）——**第二十轮（2026-09-17 晚）
 *     由现场探针 8 逐脚读数定标**（见文末「第二十轮」小节）。
 ...
 * =============================================================================
 *  第二十一轮（2026-09-17 夜）：**`prepare` 逐链掩码缺陷修复（本文件的关键修复）**
 * =============================================================================
 *  现场症状（用户回报，与第二十轮 §1 的旧读数逐字相同）：
 *    · 红「口」只亮**上半屏**（y≥8 全黑）；绿「口」**全屏一点不亮**；
 *    · 探针 8 读数不变（k=1 上半绿 / k=3 下半红 / k=4 下半绿 / k=2 与 k=5..9 无反应）。
 *  现场取证（J-Link 只读 dump 板载 flash + CCMRAM/SRAM，未改任何目标状态）：
 *    · **镜像已是新表**：运行时 `s_out_lines=0x01B`（R1|G1|R2|G2）、
 *      `s_chain_role={RED,GREEN,RED,GREEN}`、`s_chain_sel={1,2,4,8}`、
 *      `chain_dst[4][128]` 与新表逐位一致（4 链双射、链 2/3 落点全在 y≥8）；
 *    · **但帧状态数组只有第 0 位**：`_22_1665_frame_state` = 23 个 bit0（正是「口」上半屏
 *      的 23 点）、bit2 = 0（下半屏 29 点一个都没有）⇒ 屏上自然只亮上半屏；绿内容时
 *      链 0（RED 角色）取分量 = 0 ⇒ 全黑。
 *  根因（源码级，**一行**）：`_22_1665_prepare()` 多链分支原为
 *      `s |= (_22_1665_role_bit[role][c] & s_chain_sel[i]);`
 *    `_22_1665_role_bit` 的取值只有 0/1，而 `s_chain_sel[i] = 1 << i`：
 *      i=0 → 1 & 1 = 1 ✓；i=1 → 1 & 2 = **0**；i=2 → 1 & 4 = **0**；i=3 → 1 & 8 = **0**
 *    ⇒ **状态字节永远只有第 0 位** ⇒ scan/合并写表只会驱动链 0（R1）——
 *    与链表标定（脚/角色/区域）**完全无关**，这正是「改了链表行为也不变」的原因。
 *  修复：改为「该链该位点亮 → 置状态字节第 i 位」：
 *      `if (_22_1665_role_bit[s_chain_role[i]][c] != 0U) s |= s_chain_sel[i];`
 *  教训：宿主模型（check_round20.py）按**意图语义**（`role_bit << i`）建模，故 58/58 全绿而
 *  现场仍黑；本轮新增**源码级回归断言**（`.analysis/22_1665/round21_check.py` §A：既断言
 *  源码里不再出现 `& s_chain_sel[i]`，也用「错误语义」复算出现场两条症状作为反证）。
 *  修复后的预期（同表同连线）：红「口」= 整幅 52 点（R1 上半 23 + R2 下半 29）；
 *  绿「口」= 整幅 52 点（G1/G2）；下半屏若呈上下颠倒/左右镜像，再按 §二 用 `_22_1665_ROW_XF`。
 * =============================================================================
 *  第二十轮（2026-09-17 晚）：**现场探针 8 定标完成 —— 四链 = R1 / G1 / R2 / G2**
 * =============================================================================
 *  现场读数（`CHAIN_PROBE=8`、`DATA_LINES` 默认 0x3FF：逐脚 3s 停留 = 前 1.5s 2 Hz 闪 +
 *  后 1.5s 静态 1010… 纹理；R1 区显示被测脚序号 k：0=R1/1=G1/2=B1/3=R2/4=G2/5=B2/6..9=A..D）：
 *    · k=1（G1）→ 背景**绿色**出现在**上半屏**；
 *    · k=3（R2）→ 背景**红色**出现在**下半屏**；
 *    · k=4（G2）→ 背景**绿色**出现在**下半屏**；
 *    · k=0（R1）→ 上半屏红（内对照，与链 0 一致）；
 *    · k=2（B1）与 k=5..9（B2/A/B/C/D）→ **全程无反应**（背景不亮）。
 *  ⇒ **真链四条 = R1（上半红）/ G1（上半绿）/ R2（下半红）/ G2（下半绿）**；
 *     B1/B2/A/B/C/D 六根脚不与任何链相连（第十七轮的 A/B/C 推定、第十八轮的 {G2,B2,D}
 *     单脚解 **均被本轮直接读数取代**）。
 *
 *  三条历史观测在本轮一并解释清楚：
 *    · `CHAIN_PROBE=0`（红「口」只在上半屏）：旧表把上半绿挂 A、下半红挂 B、下半绿挂 C
 *      ⇒ 红内容只喂 R1（真链）与 B（死脚）⇒ 上半红出、下半不出；
 *    · `CHAIN_PROBE=5`（上半屏红全亮、绿一颗不亮）：多链模式下探针 5 的**状态位只有链 0**
 *      （`st = 1` → `st_bits` = 链 0 的 `line_mask` = R1）⇒ 只有 R1 被拉高 —— 与链表怎么填无关；
 *    · 探针 8 的参与脚 = `DATA_LINES | R1`、状态位直接由被测脚序号给出，
 *      **绕开链表逐脚驱动** ⇒ 才能看到 G1/R2/G2 的真相。
 *
 *  本轮改动：链表四行 = R1/G1/R2/G2（上半红/上半绿/下半红/下半绿，`flags` 暂恒等）；
 *  `_22_1665_CHAIN_PROBE` 默认值改回 `0U`。**仍待现场确认**：下半屏两条链（R2/G2）的
 *  片内位序/块栅格**朝向** —— 探针 8 是均匀填充，读不出朝向（现场复验清单见 doc/01 §21）。
 * =============================================================================
 *
 ...
 *
 *  ★★ 下面 4 行 = **第二十轮现场探针 8 定标结果**（2026-09-17 晚，逐脚实测读数）：
 *     链 0/1 = R1/G1（上半屏，红/绿 die）、链 2/3 = R2/G2（下半屏，红/绿 die）；
 *     **B1/B2/A/B/C/D 六根脚现场实测无链**（探针 8 逐脚「闪 + 纹理」全程无反应）⇒ 不得占用。
 *     ★仍待现场确认★：下半屏两条链（R2/G2）的**片内位序/块栅格朝向** —— 探针 8 是均匀填充，
 *     读不出朝向；若下半屏内容上下颠倒/左右镜像，把链 2/3 的 `_22_1665_ROW` 换成
 *     `_22_1665_ROW_XF(..., _22_1665_XF_ROT180)`（或对应镜像旗标）。详见 doc/01 §21。★★
 * ================================================================ */
typedef struct {
...
#if _22_1665_MULTI_CHAIN
    /* 链 0 ── ★现场三度确认（第十二/十七/二十轮）★ ── 数据脚 R1、红 die、上半屏（16×8） */
    _22_1665_ROW("C0", _22_1665_LINE_R1, _22_1665_ROLE_RED, 0U, 0U, _22_1665_SCREEN_W,
                 _22_1665_HALF_H),
#if _22_1665_CHAIN_COUNT > 1
    /* 链 1 ── 第二十轮探针 8 定标：k=1（G1）→ 上半屏出**绿** ── 上半屏 绿 */
    _22_1665_ROW("C1", _22_1665_LINE_G1, _22_1665_ROLE_GREEN, 0U, 0U, _22_1665_SCREEN_W,
                 _22_1665_HALF_H),
#endif
#if _22_1665_CHAIN_COUNT > 2
    /* 链 2 ── 第二十轮探针 8 定标：k=3（R2）→ 下半屏出**红** ── 下半屏 红 */
    _22_1665_ROW("C2", _22_1665_LINE_R2, _22_1665_ROLE_RED, 0U, _22_1665_HALF_H,
                 _22_1665_SCREEN_W, _22_1665_HALF_H),
#endif
#if _22_1665_CHAIN_COUNT > 3
    /* 链 3 ── 第二十轮探针 8 定标：k=4（G2）→ 下半屏出**绿** ── 下半屏 绿
     * 注：B1（k=2）与 B2/A/B/C/D（k=5..9）在探针 8 里**全程无反应** ⇒ 这六根脚不与任何链
     *     相连，链表一个都不占用（它们是板卡上空闲/未接线的脚，不是「待定」）。 */
    _22_1665_ROW("C3", _22_1665_LINE_G2, _22_1665_ROLE_GREEN, 0U, _22_1665_HALF_H,
                 _22_1665_SCREEN_W, _22_1665_HALF_H),
#endif
 ...
 *  第二十轮（2026-09-17 晚）已用探针 8 完成四链定标 ⇒ 默认值恢复 `0U`（正常显示）。
 *  注：**多链模式下探针 1..6 只驱动「链 0」的脚**（状态位只用到 bit0 → `st_bits` = 链 0 的
 *  `line_mask`）；「十脚恒定全 1」的口径只在兼容模式（`_22_1665_MULTI_CHAIN = 0`）下成立。 */
...
static void _22_1665_prepare(dev_display_t *dev)
{
    ...
    for (uint16_t p = 0; p < _22_1665_FRAME_BITS; p++) {
        ...
        for (uint8_t i = 0; i < _22_1665_CHAIN_COUNT; i++) {
            ...
            /* ★第二十一轮修复（2026-09-17）★ 原写法 `role_bit[...] & s_chain_sel[i]` 是错的：
             * `_22_1665_role_bit` 只有 0/1，与「该链的状态位掩码 1<<i」按位与会把 i>0 的链
             * 全部掩成 0（1&2=0、1&4=0、1&8=0）⇒ **状态字节只可能有第 0 位** ⇒ scan 只会驱动
             * 链 0（R1）；其余链无论链表怎么标定都不会亮（现场症状：红「口」只上半屏、
             * 绿「口」全黑、探针 8 正常）。正确语义 = 「该时钟位该链点亮 → 置状态字节第 i 位」。
             * 宿主自检 check_round20.py 之所以漏掉：它按**意图语义**（role_bit << i）建模，
             * 没有逐字核对 C 表达式——本轮已加源码级回归断言（round21_check.py §A）。 */
