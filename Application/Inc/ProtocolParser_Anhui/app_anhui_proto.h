...
/** 动态显示参数（0x86~0x89；动态显示功能当前留空，仅解析占位）。
 *  帧数据布局：运动模式(1B) + 速度(1B，单位 ×2ms) + 停留时间(2B，单位 ×100ms) + 文本。
 *  停留时间字节序按文档示例「00 10」取大端（=16×100ms=1.6s），待联调确认。 */
typedef struct {
    uint8_t row;      /* 行号 0~3（命令 86=第 1 行 … 89=第 4 行） */
    uint8_t mode;     /* 运动模式：0=停止 */
    uint8_t speed;    /* 速度 ×2ms */
    uint16_t stay_ms; /* 停留时间 ×100ms（大端） */
    const uint8_t *text;
    uint16_t text_len;
} anhui_scroll_t;
