 ...
 * 灯控：`0A 36/37/38/39 0D`（红/绿/黄闪开/黄闪关；黄闪开 10 秒自动关闭，对齐 9K1F212701）
 * 亮度：`0A 40 XX YY 0D`（XX 00~07 亮度，00 最暗、07 最亮，YY 保留）；心跳：`0A 50 0D`
 * 上行应答：`0A 00 0D` 正常 / `0A 01 0D` 数据超长 / `0A 02 0D` 帧错误。
 * 语义对齐参考项目 9K1F212701（etc.c cmd_etc_disPlay_ctrl / weight.c netWeinghtHandler）：
...
typedef enum {
    ...
    SC_ETC_PARSE_ERR_TOOLONG, /**< 数据超长（单行 >24 / 全屏 >145 字节，参考 9K1F212701 etc.c） */
} sc_etc_parse_sta_t;
...
typedef struct {
    ...
    uint8_t row;           /**< 0=全屏，1~6=第 1~6 行（参考 9K1F212701 支持 6 行） */
...
/**
 * @brief  黄闪 10 秒自动关闭计时启动（0x38 收到时调用，参考 9K1F212701 EtcHsTime=10）。
 */
void sc_etc_hs_timer_arm(void);

/**
 * @brief  黄闪 10 秒自动关闭计时取消（0x39 收到时调用，参考 9K1F212701 EtcHsTime=0）。
 */
void sc_etc_hs_timer_cancel(void);

/**
 * @brief  黄闪 10 秒自动关闭到期回调（由心跳计时任务调用，参考 9K1F212701 timer.c）。
 */
