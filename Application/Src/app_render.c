...
msl_addr_part_t addr_part_table[3] = {
    [0] = {96, 24},
    [1] = {48, 48},
    [2] = {0, 48},
};
...
void msl_render_text(const render_cfg_t *cfg)
{
    ...
    while (char_pos < text_len) {
        ...
        if (text_buf[char_pos] >= 0x20 && text_buf[char_pos] <= 0x7F) {
            ...
            uint8_t ch_byte = (uint8_t)text_buf[char_pos];
            uint32_t addr   = _flash_addr(font_size, FONT_ENC_ASCII, cfg->font_type, &ch_byte);
            dev_storage_read(s_render_font, addr, font_buf, _packed_glyph_bytes(font_size, FONT_ENC_ASCII));
            dev_display_fill(s_render_display, cur_x, cur_y, glyph_w, font_size, COLOR_BLACK);
            dev_display_draw_bitmap(&msl_ins, cur_x, cur_y, glyph_w, font_size, font_buf, cfg->color);
            ...
            uint8_t gbk_ch[2] = {(uint8_t)text_buf[char_pos], (uint8_t)text_buf[char_pos + 1]};
            uint32_t addr     = _flash_addr(font_size, FONT_ENC_GBK, cfg->font_type, gbk_ch);
            dev_storage_read(s_render_font, addr, font_buf, _packed_glyph_bytes(font_size, FONT_ENC_GBK));
            dev_display_fill(s_render_display, cur_x, cur_y, glyph_w, font_size, COLOR_BLACK);
            dev_display_draw_bitmap(&msl_ins, cur_x, cur_y, glyph_w, font_size, font_buf, cfg->color);
...
/**
 * @brief 多控制卡显示同步，通过msl协议实现多控制卡同步显示单屏幕
 *
 * @param dsp 显示实例
 * @param addr 控制卡地址，0x00为广播，0xff保留
 * @param ctx 显示内容配置的上下文
 * @param pers 是否启用持久化
 */
void app_bitmap_sychro(dev_display_t *dsp, uint8_t addr, render_cfg_t ctx, bool pers)
{
    uint16_t cols = ctx.h, rows = ctx.w;
    uint8_t color      = COLOR_BLACK;
    uint16_t row_bytes = (rows + 7) / 8;
    uint16_t bm_bytes  = cols * row_bytes;

    // 构造同步显示帧
    static uint8_t send_buf[MSL_PAYLOAD_MAX];
    msl_frame_t *f      = (msl_frame_t *)send_buf;
    msl_bitmap_t *f_ctx = (msl_bitmap_t *)(f->data_bcc);
    memset(send_buf, 0, sizeof(send_buf));

    f->head[0]        = 0x5a;
    f->head[1]        = 0x5a;
    f->addr           = addr;
    f->cmd            = MSL_CMD_BITMAP;
    uint16_t data_len = sizeof(msl_bitmap_t) + bm_bytes;
    f->length[0]      = (data_len >> 8) & 0xff;
    f->length[1]      = data_len & 0xff;

    f_ctx->width = ctx.w;
    f_ctx->high  = ctx.h;
    f_ctx->color = ctx.color;
    f_ctx->pers  = pers;
