...
/* 队列深度 1，按本地惯例 depth × msg_size 精确分配（FreeRTOS 静态队列要求 mq_size >= depth × msg_size） */
static uint8_t s_msl_queue_buf[MSL_MSG_SIZE];
...
{
    /* 仅 CPU memcpy 访问（osMessageQueueGet），放 CCMRAM 省 SRAM */
    [[gnu::section(".ccmram")]] static uint8_t _msg_buf[MSL_MSG_SIZE];
...
proto_probe_sta_t msl_probe_frame(const channel_t *ch, const ring_buffer_t *buff, uint32_t *total_len, uint8_t *aux)
{
    ...
    /* 仅 CPU memcpy 访问（rb_peek），放 CCMRAM 省 SRAM */
    [[gnu::section(".ccmram")]] static uint8_t mem_pool[MSL_MSG_SIZE] = {0};
    memset(mem_pool, 0, sizeof(mem_pool));
    /* 探测缓冲固定 MSL_MSG_SIZE：peek 长度封顶防越界（avail 可达 RB 容量 > 缓冲） */
    uint32_t peek_len = (avail > MSL_MSG_SIZE) ? MSL_MSG_SIZE : avail;
