 ...
 * 心跳机制由固件实现：独立计时任务，5 分钟未收到任何有效帧（心跳/显示/灯控/亮度）
 ...
 * 参考 9K1F212701 USER/ETC/etc.c `cmd_etc_disPlay_ctrl`：0x0D 定界扫描上限
 * `i > 148` 即拒 → 0x0D 最迟位于索引 148 → 帧总长 ≤149B（头 3 + 数据 ≤145 + 0D）。
...
/** ETC 黄闪自动关闭时长（参考 9K1F212701 etc.c：EtcHsTime=10，1s 递减到 0 关黄闪） */
...
/** @brief 0x38 黄闪开：启动 10 秒自动关闭计时（参考 9K1F212701 EtcHsTime=10）。 */
void sc_etc_hs_timer_arm(void)
{
    s_etc_hs_off_s = s_etc_uptime_s + SC_ETC_HS_TIMEOUT_S;
}

/** @brief 0x39 黄闪关：清零自动关闭计时（参考 9K1F212701 EtcHsTime=0）。 */
 ...
 *         并处理 ETC 黄闪 10 秒自动关闭（参考 9K1F212701 timer.c）。
 ...
 *     （≤149 字节；9K1F212701 etc.c cmd_etc_disPlay_ctrl：0x0D 索引 i>148 拒帧）
                                            ...
                                            uint32_t *total_len, uint8_t *aux)
{
    ...
    switch (cmd_byte) {
        ...
        case 0x01: { /* 滚屏显示 */
            ...
            if (row > 6U) /* 参考 9K1F212701：单行行号 1~6，0=全屏 */
                return PROTO_PROBE_FAKE;

            /* 变长数据：扫描 0x0D 结尾；上限 149 字节
             * （9K1F212701 etc.c cmd_etc_disPlay_ctrl：扫描索引 i>148 即拒 →
             *   0x0D 最迟索引 148 → 帧 ≤149B；数据段无固定 56B 长度）。
...
void sc_etc_proto_handle_task(void *argument)
{
    ...
    for (;;) {
        ...
        if (cmd.sta == SC_ETC_PARSE_OK) {
            sc_etc_activity_refresh(); /* 任意有效帧刷新心跳计时 */
        }
